...
Write a query that returns: username and password for all waiters which are not fired (fired waiter is a waiter without salary). The username is generated by their last name immediately followed by their first name  followed by the number of characters from first name and at the end "Restaurant". The password is their email starting from the 2nd character to the 13th character and then reversed.
